...
export default {
  data(){
    ...
    methods:{
      ...
      {
        var pre=this.getPrevPageArray;
        // pageCounter*=10;
        pageCounter = pre.length;        
        // console.log(pre.length)
        var prev=[];
        var k=0;
        for(var i=pageCounter-4;i<=pageCounter-3;i++)
        {
          prev[k]=pre[i];
          k++;
        }
    ...
    filters:
    {
      ImageUrl(value)
      {
        ...
        // console.log(value);
        return process.env.VUE_APP_SERVER+"/seller/"+value.split(process.env.VUE_APP_SERVER+"/")[1];
      }
    }
}
</script>
